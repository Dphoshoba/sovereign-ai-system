...
export class SandboxExecutionPipeline {
  ...
  async execute(
    ...
    try {
      mutationResult = await this.config.adapter.execute(request, candidate);
    ...
    if (!verificationPassed) {
      ...
      // Attempt compensating rollback on verification failure
