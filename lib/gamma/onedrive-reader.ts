...
import type { OneDriveResource } from "../connectors/onedrive/resource-parser";

export class OneDriveReader extends GammaReaderBase<OneDriveResource> {
  getByName(name: string, _currentTime: Date): OneDriveResource | undefined {
    return this.find((resource) => resource.name === name);
  }

  getByType(resourceType: OneDriveResource["resourceType"]): OneDriveResource[] {
    return this.filter((resource) => resource.resourceType === resourceType);
  }

  getRecent(currentTime: Date, limitDays = 7): OneDriveResource[] {
