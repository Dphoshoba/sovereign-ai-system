...
export function getSalesforceProductionReadiness(
...
export function getSalesforceRetryPolicy(): {
  connectorId: "salesforce";
...
export function projectSalesforceHealth(params: {
  currentTime: Date;
  ...
  if (params.quotaUsedPercent >= 90) warnings.push("Salesforce quota is above 90%.");
  if (tokenMinutesUntilExpiry < 10) {
    warnings.push("Salesforce token expires in less than 10 minutes.");
  }
