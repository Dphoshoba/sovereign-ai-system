...
export function getStripeRetryPolicy(): {
  connectorId: "stripe";
...
export function projectStripeHealth(params: {
  currentTime: Date;
  ...
  if (params.quotaUsedPercent >= 90) warnings.push("Stripe quota is above 90%.");
  if (tokenMinutesUntilExpiry < 10) {
    warnings.push("Stripe token expires in less than 10 minutes.");
  }
