...
export function getSharePointProductionReadiness(
...
export function getSharePointRetryPolicy(): {
  connectorId: "sharepoint";
...
export function projectSharePointHealth(params: {
  currentTime: Date;
  ...
  if (params.quotaUsedPercent >= 90) warnings.push("SharePoint quota is above 90%.");
  if (tokenMinutesUntilExpiry < 10) {
    warnings.push("SharePoint token expires in less than 10 minutes.");
  }
