...
export function getHubSpotProductionReadiness(
...
export function getHubSpotRetryPolicy(): {
  connectorId: "hubspot";
...
export function projectHubSpotHealth(params: {
  currentTime: Date;
  ...
  if (params.quotaUsedPercent >= 90) warnings.push("HubSpot quota is above 90%.");
  if (tokenMinutesUntilExpiry < 10) {
    warnings.push("HubSpot token expires in less than 10 minutes.");
  }
