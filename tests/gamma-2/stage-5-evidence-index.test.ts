...
describe("Gamma 2 Stage 5 evidence index", () => {
  it("builds an indexed catalog of Stage 5 evidence artifacts", () => {
    ...
    expect(index.apiSurfaceCount).toBe(22);
    expect(index.smoke).toBe("45 routes passed, 0 failed");
    expect(index.entries).toHaveLength(22);
  ...
  it("serves the index through the Stage 5 evidence index route", async () => {
    ...
    expect(body.apiSurfaceCount).toBe(22);
    expect(body.entries).toHaveLength(22);
  });
});
