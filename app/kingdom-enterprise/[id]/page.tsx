...
import { getMissionKingdomEnterprise } from "../../../lib/gamma/kingdom-enterprise-reader"

export const dynamic = "force-dynamic"

export default async function KingdomEnterpriseMissionPage({
  params,
}: {
  params: Promise<{ id: string }>
}) {
  const { id } = await params
  const workspace = await getMissionKingdomEnterprise(id)

  if (!workspace) {
    return (
      <main style={pageStyle}>
        <article style={cardStyle}>
          <h1 style={titleStyle}>Enterprise Not Found</h1>
          <p style={mutedStyle}>No enterprise data exists for mission {id}.</p>
          <Link href="/kingdom-enterprise" style={linkStyle}>Back to Kingdom Enterprise</Link>
        </article>
      </main>
    )
  }
  ...
  return (
    <main style={pageStyle}>
      ...
      <section style={metricsStyle}>
        ...
        <Metric label="Scale Score" value={String(workspace.scaleScore)} />
        <Metric label="Discipline Score" value={String(workspace.disciplineScore)} />
        <Metric label="Leadership Score" value={String(workspace.leadershipScore)} />
        <Metric label="Operational Score" value={String(workspace.operationalScore)} />
        <Metric label="Financial Score" value={String(workspace.financialScore)} />
        <Metric label="Cultural Score" value={String(workspace.culturalScore)} />
      </section>
