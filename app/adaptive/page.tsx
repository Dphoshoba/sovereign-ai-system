...
import { getAdaptiveRegistry } from "../../lib/gamma/adaptive-reader"

export const dynamic = "force-dynamic"

export default async function AdaptivePage() {
  const registry = await getAdaptiveRegistry()

  return (
    <main style={pageStyle}>
      <section style={cardStyle}>
        <p style={eyebrowStyle}>Adaptive Engine</p>
        <h1 style={titleStyle}>System Adaptation</h1>
        <p style={mutedStyle}>Dynamic system adaptation and self-improvement.</p>
      </section>

      <section style={metricsStyle}>
        <Metric label="Adaptation Score" value={String(registry.adaptationScore)} />
        <Metric label="Change Velocity" value={String(registry.changeVelocity)} />
        <Metric label="Learning Capacity" value={String(registry.learningCapacity)} />
        <Metric label="Self Improvement" value={String(registry.selfImprovement)} />
      ...
      <section style={gridStyle}>
        {registry.missions.map((mission) => (
          <article key={mission.mission} style={cardStyle}>
            ...
            <p style={mutedStyle}>Adaptation: {mission.adaptationScore}</p>
            <Link href={`/adaptive/${mission.mission}`} style={linkStyle}>View adaptive</Link>
          </article>
        ))}
      </section>
    </main>
