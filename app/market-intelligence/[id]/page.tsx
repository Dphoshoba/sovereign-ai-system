...
import { getMissionMarketIntelligence } from "../../../lib/gamma/market-intelligence-reader"

export const dynamic = "force-dynamic"

export default async function MarketIntelligenceMissionPage({
  params,
}: {
  params: Promise<{ id: string }>
}) {
  const { id } = await params
  const workspace = await getMissionMarketIntelligence(id)

  if (!workspace) {
    return (
      <main style={pageStyle}>
        <article style={cardStyle}>
          <h1 style={titleStyle}>Market Not Found</h1>
          <p style={mutedStyle}>No market intelligence workspace exists for mission {id}.</p>
          <Link href="/market-intelligence" style={linkStyle}>Back to Market</Link>
        </article>
      </main>
    )
  }
  ...
  return (
    <main style={pageStyle}>
      ...
      <section style={metricsStyle}>
        <Metric label="Market Segments" value={String(workspace.marketSegments)} />
        <Metric label="Audience Count" value={String(workspace.audienceCount)} />
        <Metric label="Demand Score" value={String(workspace.demandScore)} />
        <Metric label="Competition Score" value={String(workspace.competitionScore)} />
      ...
      <section style={cardStyle}>
        <h2 style={sectionTitleStyle}>Market Segments</h2>
        <ul style={listStyle}>
          {workspace.segments.map((s) => (
            <li key={s.segmentId} style={listItemStyle}>
              {s.segmentName}: Size {s.audienceSize}, Demand {s.demandLevel}, Competition {s.competitionLevel}
            </li>
          ))}
        </ul>
      </section>
