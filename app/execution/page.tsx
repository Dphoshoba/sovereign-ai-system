...
import { getExecutionRegistry } from "../../lib/gamma/execution-reader"
  ...
  return (
    <main style={pageStyle}>
      <section style={cardStyle}>
        ...
        <h1 style={titleStyle}>Mission Execution</h1>
        <p style={mutedStyle}>Deterministic execution tracking across task capacity, velocity, and delivery.</p>
      </section>

      <section style={metricsStyle}>
        <Metric label="Task Count" value={String(registry.taskCount)} />
        <Metric label="Active Projects" value={String(registry.activeProjects)} />
        <Metric label="Execution Capacity" value={String(registry.executionCapacity)} />
        <Metric label="Execution Velocity" value={String(registry.executionVelocity)} />
        <Metric label="Bottlenecks" value={String(registry.bottlenecks)} />
        <Metric label="Blocked Items" value={String(registry.blockedItems)} />
        <Metric label="Focus Score" value={String(registry.focusScore)} />
        <Metric label="Delivery Score" value={String(registry.deliveryScore)} />
      ...
      <section style={gridStyle}>
        {registry.missions.map((mission) => (
          <article key={mission.mission} style={cardStyle}>
            ...
            <p style={mutedStyle}>Execution Velocity: {mission.executionVelocity}</p>
            <p style={mutedStyle}>Delivery Score: {mission.deliveryScore}</p>
            <Link href={`/execution/${mission.mission}`} style={linkStyle}>Open execution</Link>
          </article>
        ))}
      </section>
    </main>
