...
import { getCollectiveIntelligenceRegistry } from "../../lib/gamma/collective-intelligence-reader"
...
export default async function CollectiveIntelligencePage() {
  const registry = await getCollectiveIntelligenceRegistry()

  return (
    <main style={pageStyle}>
      <section style={cardStyle}>
        <h1>Collective Intelligence Engine</h1>
        <p>Cross-mission learning and reasoning</p>
      </section>

      <section style={metricsStyle}>
        <div style={metricBoxStyle}>
          <div style={{ fontSize: "12px", opacity: 0.6 }}>Collective Score</div>
          <div style={{ fontSize: "24px", fontWeight: "bold" }}>{registry.collectiveScore}</div>
        </div>
        <div style={metricBoxStyle}>
          <div style={{ fontSize: "12px", opacity: 0.6 }}>Cross-Mission Learning</div>
          <div style={{ fontSize: "24px", fontWeight: "bold" }}>{registry.crossMissionLearning}</div>
        </div>
        <div style={metricBoxStyle}>
          <div style={{ fontSize: "12px", opacity: 0.6 }}>Knowledge Reuse</div>
          <div style={{ fontSize: "24px", fontWeight: "bold" }}>{registry.knowledgeReuse}</div>
        </div>
        <div style={metricBoxStyle}>
          <div style={{ fontSize: "12px", opacity: 0.6 }}>Adaptation Capacity</div>
          <div style={{ fontSize: "24px", fontWeight: "bold" }}>{registry.adaptationCapacity}</div>
        </div>
        <div style={metricBoxStyle}>
          <div style={{ fontSize: "12px", opacity: 0.6 }}>Coordination Score</div>
          <div style={{ fontSize: "24px", fontWeight: "bold" }}>{registry.coordinationScore}</div>
        </div>
        <div style={metricBoxStyle}>
        ...
        {registry.missions.map((mission) => (
          <div key={mission.mission} style={missionCardStyle}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", marginTop: "8px" }}>
              <span>Score: {mission.collectiveScore}</span>
