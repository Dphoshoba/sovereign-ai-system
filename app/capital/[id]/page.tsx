...
import { getMissionCapital } from "../../../lib/gamma/capital-reader"

export const dynamic = "force-dynamic"

export default async function CapitalMissionPage({
  params,
}: {
  params: Promise<{ id: string }>
}) {
  const { id } = await params
  const workspace = await getMissionCapital(id)

  if (!workspace) {
    return (
      <main style={pageStyle}>
        <article style={cardStyle}>
          <h1 style={titleStyle}>Capital View Not Found</h1>
          <p style={mutedStyle}>No capital workspace exists for mission {id}.</p>
          <Link href="/capital" style={linkStyle}>Back to Capital</Link>
        </article>
      </main>
    )
  }
  ...
  return (
    <main style={pageStyle}>
      ...
      <section style={metricsStyle}>
        ...
        <Metric label="Licensing Score" value={String(workspace.licensingScore)} />
        <Metric label="Content Score" value={String(workspace.contentScore)} />
        <Metric label="Health Score" value={String(workspace.healthScore)} />
      </section>

      <section style={twoColStyle}>
        <article style={cardStyle}>
          <h2 style={sectionTitleStyle}>Capital Assets</h2>
          <ul style={listStyle}>
            {workspace.assets.map((item) => (
              <li key={item.title} style={listItemStyle}>{item.title} ({item.category}): {item.value}</li>
            ))}
          </ul>
        </article>
