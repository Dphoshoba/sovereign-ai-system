...
import { getScenarioRegistry } from "../../lib/gamma/scenario-reader"

export const dynamic = "force-dynamic"

export default async function ScenarioPage() {
  const registry = await getScenarioRegistry()

  return (
    <main style={pageStyle}>
      <section style={cardStyle}>
        <p style={eyebrowStyle}>Scenario Engine</p>
        <h1 style={titleStyle}>Scenario Planning</h1>
        <p style={mutedStyle}>Best, worst, and expected case scenarios.</p>
      </section>

      <section style={metricsStyle}>
        <Metric label="Scenario Count" value={String(registry.scenarioCount)} />
        <Metric label="Best Case" value={String(registry.bestCase)} />
        <Metric label="Worst Case" value={String(registry.worstCase)} />
        <Metric label="Expected Case" value={String(registry.expectedCase)} />
      ...
      <section style={gridStyle}>
        {registry.missions.map((mission) => (
          <article key={mission.mission} style={cardStyle}>
            ...
            <p style={mutedStyle}>Expected: {mission.expectedCase}%</p>
            <Link href={`/scenario/${mission.mission}`} style={linkStyle}>View scenario</Link>
          </article>
        ))}
      </section>
    </main>
