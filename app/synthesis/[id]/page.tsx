...
import { getMissionSynthesis } from "../../../lib/gamma/synthesis-reader"

export const dynamic = "force-dynamic"

export default async function SynthesisMissionPage({
  params,
}: {
  params: Promise<{ id: string }>
}) {
  const { id } = await params
  const workspace = await getMissionSynthesis(id)

  if (!workspace) {
    return (
      <main style={pageStyle}>
        <article style={cardStyle}>
          <h1 style={titleStyle}>Synthesis Not Found</h1>
          <p style={mutedStyle}>No synthesis workspace exists for mission {id}.</p>
          <Link href="/synthesis" style={linkStyle}>Back to Synthesis</Link>
        </article>
      </main>
    )
  }
  ...
  return (
    <main style={pageStyle}>
      <section style={cardStyle}>
        <p style={eyebrowStyle}>Synthesis Engine</p>
        <h1 style={titleStyle}>Research Mission 001</h1>
        <p style={mutedStyle}>{workspace.missionTitle}</p>
      </section>

      <section style={metricsStyle}>
        <Metric label="Cross-Domain Links" value={String(workspace.crossDomainLinks)} />
        <Metric label="Synthesis Count" value={String(workspace.synthesisCount)} />
        <Metric label="Knowledge Density" value={String(workspace.knowledgeDensity)} />
        <Metric label="Integration Score" value={String(workspace.integrationScore)} />
        <Metric label="Reuse Score" value={String(workspace.reuseScore)} />
        <Metric label="Health Score" value={String(workspace.healthScore)} />
      </section>

      <section style={cardStyle}>
        <h2 style={sectionTitleStyle}>Knowledge Links</h2>
        <ul style={listStyle}>
          {workspace.links.map((l) => (
            <li key={l.linkId} style={listItemStyle}>
              {l.linkName}: Integration {l.integrationScore}, Reuse {l.reuseScore}, Density {l.knowledgeDensity}
            </li>
          ))}
        </ul>
      </section>
