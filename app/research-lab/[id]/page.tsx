...
import { getMissionResearchLab } from "../../../lib/gamma/research-lab-reader"

export const dynamic = "force-dynamic"

export default async function ResearchLabMissionPage({
  params,
}: {
  params: Promise<{ id: string }>
}) {
  const { id } = await params
  const workspace = await getMissionResearchLab(id)

  if (!workspace) {
    return (
      <main style={pageStyle}>
        <article style={cardStyle}>
          <h1 style={titleStyle}>Research Lab Not Found</h1>
          <p style={mutedStyle}>No research lab workspace exists for mission {id}.</p>
          <Link href="/research-lab" style={linkStyle}>Back to Lab</Link>
        </article>
      </main>
    )
  }
  ...
  return (
    <main style={pageStyle}>
      ...
      <section style={metricsStyle}>
        <Metric label="Research Projects" value={String(workspace.researchProjects)} />
        <Metric label="Discoveries" value={String(workspace.discoveries)} />
        <Metric label="Frameworks" value={String(workspace.frameworks)} />
        <Metric label="Publication Score" value={String(workspace.publicationScore)} />
        <Metric label="Experimentation Score" value={String(workspace.experimentationScore)} />
        <Metric label="Health Score" value={String(workspace.healthScore)} />
      </section>

      <section style={cardStyle}>
        <h2 style={sectionTitleStyle}>Research Projects</h2>
        <ul style={listStyle}>
          {workspace.projects.map((p) => (
            <li key={p.projectId} style={listItemStyle}>
              {p.projectName}: Discoveries {p.discoveryCount}, Framework {p.frameworkScore}, Experimentation {p.experimentationLevel}
            </li>
          ))}
        </ul>
      </section>
