...
import { getImpactRegistry } from "../../lib/gamma/impact-reader"

export const dynamic = "force-dynamic"

export default async function ImpactPage() {
  const registry = await getImpactRegistry()

  return (
    <main style={pageStyle}>
      <section style={cardStyle}>
        <p style={eyebrowStyle}>Impact Engine</p>
        <h1 style={titleStyle}>Mission Impact</h1>
        <p style={mutedStyle}>Societal value and legacy measurement.</p>
      </section>

      <section style={metricsStyle}>
        <Metric label="Impact Areas" value={String(registry.impactAreas)} />
        <Metric label="People Reached" value={String(registry.peopleReached)} />
        <Metric label="Mission Influence" value={String(registry.missionInfluence)} />
        <Metric label="Societal Value" value={String(registry.societalValue)} />
        <Metric label="Legacy Score" value={String(registry.legacyScore)} />
      ...
      <section style={gridStyle}>
        {registry.missions.map((mission) => (
          <article key={mission.mission} style={cardStyle}>
            ...
            <p style={mutedStyle}>Impact: {mission.impactScore}</p>
            <Link href={`/impact/${mission.mission}`} style={linkStyle}>View impact</Link>
          </article>
        ))}
      </section>
    </main>
