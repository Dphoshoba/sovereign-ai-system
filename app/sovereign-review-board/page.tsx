...
import { getSovereignReviewBoardRegistry } from "../../lib/gamma/sovereign-review-board-reader"
...
export default async function SovereignReviewBoardPage() {
  const registry = await getSovereignReviewBoardRegistry()

  return (
    <main style={pageStyle}>
      <section style={cardStyle}>
        <h1>Sovereign Review Board</h1>
        <p>Governance transparency and accountability</p>
      </section>

      <section style={metricsStyle}>
        <div style={metricBoxStyle}>
          <div style={{ fontSize: "12px", opacity: 0.6 }}>Review Score</div>
          <div style={{ fontSize: "24px", fontWeight: "bold" }}>{registry.reviewScore}</div>
        </div>
        <div style={metricBoxStyle}>
          <div style={{ fontSize: "12px", opacity: 0.6 }}>Governance</div>
          <div style={{ fontSize: "24px", fontWeight: "bold" }}>{registry.governance}</div>
        </div>
        <div style={metricBoxStyle}>
          <div style={{ fontSize: "12px", opacity: 0.6 }}>Transparency</div>
          <div style={{ fontSize: "24px", fontWeight: "bold" }}>{registry.transparency}</div>
        </div>
        <div style={metricBoxStyle}>
          <div style={{ fontSize: "12px", opacity: 0.6 }}>Accountability</div>
          <div style={{ fontSize: "24px", fontWeight: "bold" }}>{registry.accountability}</div>
        </div>
        <div style={metricBoxStyle}>
          <div style={{ fontSize: "12px", opacity: 0.6 }}>Oversight</div>
          <div style={{ fontSize: "24px", fontWeight: "bold" }}>{registry.oversight}</div>
        </div>
        <div style={metricBoxStyle}>
          <div style={{ fontSize: "12px", opacity: 0.6 }}>Authority</div>
          <div style={{ fontSize: "24px", fontWeight: "bold" }}>{registry.authority}</div>
        </div>
        <div style={metricBoxStyle}>
        ...
        {registry.missions.map((mission) => (
          <div key={mission.mission} style={missionCardStyle}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", marginTop: "8px" }}>
              <span>Review: {mission.reviewScore}</span>
