...
import { getMetaIntelligenceRegistry } from "../../lib/gamma/meta-intelligence-reader"

export const dynamic = "force-dynamic"

export default async function MetaIntelligencePage() {
  const registry = await getMetaIntelligenceRegistry()

  return (
    <main style={pageStyle}>
      <section style={cardStyle}>
        <p style={eyebrowStyle}>Meta Intelligence</p>
        <h1 style={titleStyle}>Meta-Cognitive Intelligence</h1>
        <p style={mutedStyle}>System self-awareness and reasoning.</p>
      </section>

      <section style={metricsStyle}>
        <Metric label="Meta Score" value={String(registry.metaScore)} />
        <Metric label="System Awareness" value={String(registry.systemAwareness)} />
        <Metric label="Cross-Domain Reasoning" value={String(registry.crossDomainReasoning)} />
        <Metric label="Optimization Depth" value={String(registry.optimizationDepth)} />
      ...
      <section style={gridStyle}>
        {registry.missions.map((mission) => (
          <article key={mission.mission} style={cardStyle}>
            ...
            <p style={mutedStyle}>Meta: {mission.metaScore}</p>
            <Link href={`/meta-intelligence/${mission.mission}`} style={linkStyle}>View meta</Link>
          </article>
        ))}
      </section>
    </main>
