...
import { getMissionMetaIntelligence } from "../../../lib/gamma/meta-intelligence-reader"

export const dynamic = "force-dynamic"

export default async function MetaIntelligenceMissionPage({
  params,
}: {
  params: Promise<{ id: string }>
}) {
  const { id } = await params
  const workspace = await getMissionMetaIntelligence(id)

  if (!workspace) {
    return (
      <main style={pageStyle}>
        <article style={cardStyle}>
          <h1 style={titleStyle}>Meta Intelligence Not Found</h1>
          <p style={mutedStyle}>No meta intelligence workspace exists for mission {id}.</p>
          <Link href="/meta-intelligence" style={linkStyle}>Back to Meta</Link>
        </article>
      </main>
    )
  }
  ...
  return (
    <main style={pageStyle}>
      <section style={cardStyle}>
        <p style={eyebrowStyle}>Meta Intelligence</p>
        <h1 style={titleStyle}>Research Mission 001</h1>
        <p style={mutedStyle}>{workspace.missionTitle}</p>
      </section>

      <section style={metricsStyle}>
        <Metric label="Meta Score" value={String(workspace.metaScore)} />
        <Metric label="System Awareness" value={String(workspace.systemAwareness)} />
        <Metric label="Cross-Domain Reasoning" value={String(workspace.crossDomainReasoning)} />
        <Metric label="Optimization Depth" value={String(workspace.optimizationDepth)} />
      ...
      <section style={cardStyle}>
        ...
        <ul style={listStyle}>
          {workspace.capabilities.map((c) => (
            <li key={c.capabilityId} style={listItemStyle}>
              {c.name}: Score {c.score}, Sophistication {c.sophistication}
            </li>
          ))}
        </ul>
      </section>
