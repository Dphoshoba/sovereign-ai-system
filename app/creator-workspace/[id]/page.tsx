...
export default async function CreatorWorkspaceMissionPage({
  params,
}: {
  params: Promise<{ id: string }>
}) {
  const { id } = await params
  const workspace = await getCreatorWorkspace(id)
    ...
    return (
      <main style={pageStyle}>
        <section style={cardStyle}>
          ...
          <p style={bodyTextStyle}>The requested creator workspace "{id}" does not exist.</p>
        </section>
      </main>
    )
  }
    ...
    <main style={pageStyle}>
      <section style={headerStyle}>
        <div>
          ...
          <p style={subtitleStyle}>{workspace.relatedMissionName}</p>
        </div>
        <div style={statusClusterStyle}>
          <StatusPill label="Mode" value="Read-Only" />
          <StatusPill label="Content Status" value={workspace.contentStatus} />
        </div>
      </section>

      <section style={summaryGridStyle}>
        <MetricCard label="Progress" value={`${workspace.progress}%`} detail="Output conversion progress" />
        <MetricCard label="Confidence" value={workspace.confidence} detail="Preview certainty" />
        <MetricCard label="Related Research Mission" value={workspace.relatedMissionName} detail={workspace.relatedMissionId} />
      ...
      <section style={footerStyle}>
        <Link href="/creator-workspace" style={linkStyle}>
