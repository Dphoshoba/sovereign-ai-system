...
import { getOpportunityRegistry } from "../../lib/gamma/opportunity-reader"

export const dynamic = "force-dynamic"

export default async function OpportunitiesPage() {
  const registry = await getOpportunityRegistry()

  return (
    <main style={pageStyle}>
      <section style={cardStyle}>
        <p style={eyebrowStyle}>Opportunity Engine</p>
        <h1 style={titleStyle}>Mission Opportunities</h1>
        <p style={mutedStyle}>Deterministic opportunity ranking across market, education, ministry, and creator lanes.</p>
      </section>

      <section style={metricsStyle}>
        <Metric label="Opportunity Count" value={String(registry.opportunityCount)} />
        <Metric label="Market Potential" value={String(registry.marketPotential)} />
        <Metric label="Education Potential" value={String(registry.educationPotential)} />
        <Metric label="Ministry Potential" value={String(registry.ministryPotential)} />
        <Metric label="Creator Potential" value={String(registry.creatorPotential)} />
        <Metric label="ROI Score" value={String(registry.roiScore)} />
      ...
      <section style={gridStyle}>
        {registry.missions.map((mission) => (
          <article key={mission.mission} style={cardStyle}>
            ...
            <p style={mutedStyle}>Opportunity Count: {mission.opportunityCount}</p>
            <p style={mutedStyle}>ROI Score: {mission.roiScore}</p>
            <Link href={`/opportunities/${mission.mission}`} style={linkStyle}>Open opportunities</Link>
          </article>
        ))}
      </section>
    </main>
