...
import { getCreateRegistry } from "../../lib/gamma/create-reader"

export const dynamic = "force-dynamic"

export default async function CreatePage() {
  const registry = await getCreateRegistry()

  return (
    <main style={pageStyle}>
      <section style={cardStyle}>
        <p style={eyebrowStyle}>Gamma Create Command System</p>
        <h1 style={titleStyle}>Create Commands</h1>
        <p style={mutedStyle}>Deterministic command previews for mission workspace generation.</p>
      </section>

      <section style={metricsStyle}>
        <Metric label="Command Count" value={String(registry.commandCount)} />
        <Metric label="Template Coverage" value={String(registry.templateCoverage)} />
        <Metric label="Workspace Generation Score" value={String(registry.workspaceGenerationScore)} />
        <Metric label="Automation Score" value={String(registry.automationScore)} />
      ...
      <section style={gridStyle}>
        {registry.missions.map((mission) => (
          <article key={mission.mission} style={cardStyle}>
            ...
            <p style={mutedStyle}>Automation Score: {mission.automationScore}</p>
            <p style={mutedStyle}>Workspace Generation: {mission.workspaceGenerationScore}</p>
            <Link href={`/create/${mission.mission}`} style={linkStyle}>Open create workspace</Link>
          </article>
        ))}
      </section>
    </main>
