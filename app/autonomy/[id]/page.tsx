...
import { getMissionAutonomy } from "../../../lib/gamma/autonomy-reader"

export const dynamic = "force-dynamic"

export default async function AutonomyMissionPage({
  params,
}: {
  params: Promise<{ id: string }>
}) {
  const { id } = await params
  const workspace = await getMissionAutonomy(id)

  if (!workspace) {
    return (
      <main style={pageStyle}>
        <article style={cardStyle}>
          <h1 style={titleStyle}>Autonomy Not Found</h1>
          <p style={mutedStyle}>No autonomy workspace exists for mission {id}.</p>
          <Link href="/autonomy" style={linkStyle}>Back to Autonomy</Link>
        </article>
      </main>
    )
  }
  ...
  return (
    <main style={pageStyle}>
      <section style={cardStyle}>
        <p style={eyebrowStyle}>Autonomy Engine</p>
      ...
      <section style={metricsStyle}>
        ...
        <Metric label="Execution Capability" value={String(workspace.executionCapability)} />
        <Metric label="Decision Capacity" value={String(workspace.decisionCapacity)} />
        ...
        <ul style={listStyle}>
          {workspace.capabilities.map((c) => (
            <li key={c.capabilityId} style={listItemStyle}>
              {c.name}: Automation {c.automationLevel}, Readiness {c.readiness}
            </li>
          ))}
        </ul>
      </section>
