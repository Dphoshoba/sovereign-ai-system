...
import { getEcosystemRegistry } from "../../lib/gamma/ecosystem-reader"

export const dynamic = "force-dynamic"

export default async function EcosystemPage() {
  const registry = await getEcosystemRegistry()

  return (
    <main style={pageStyle}>
      <section style={cardStyle}>
        <p style={eyebrowStyle}>Ecosystem Engine</p>
        <h1 style={titleStyle}>Connected Systems</h1>
        <p style={mutedStyle}>Partner networks and ecosystem intelligence.</p>
      </section>

      <section style={metricsStyle}>
        <Metric label="Partner Count" value={String(registry.partnerCount)} />
        <Metric label="Community Assets" value={String(registry.communityAssets)} />
        <Metric label="Connected Systems" value={String(registry.connectedSystems)} />
        <Metric label="Network Strength" value={String(registry.networkStrength)} />
        <Metric label="Ecosystem Score" value={String(registry.ecosystemScore)} />
      ...
      <section style={gridStyle}>
        {registry.missions.map((mission) => (
          <article key={mission.mission} style={cardStyle}>
            ...
            <p style={mutedStyle}>Partners: {mission.partnerCount}</p>
            <Link href={`/ecosystem/${mission.mission}`} style={linkStyle}>View ecosystem</Link>
          </article>
        ))}
      </section>
    </main>
