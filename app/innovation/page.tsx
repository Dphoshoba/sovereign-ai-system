...
import { getInnovationRegistry } from "../../lib/gamma/innovation-reader"

export const dynamic = "force-dynamic"

export default async function InnovationPage() {
  const registry = await getInnovationRegistry()

  return (
    <main style={pageStyle}>
      <section style={cardStyle}>
        <p style={eyebrowStyle}>Innovation Engine</p>
        <h1 style={titleStyle}>Innovation Development</h1>
        <p style={mutedStyle}>Ideas, prototypes, and innovation tracking.</p>
      </section>

      <section style={metricsStyle}>
        <Metric label="Innovation Count" value={String(registry.innovationCount)} />
        <Metric label="New Concepts" value={String(registry.newConcepts)} />
        <Metric label="Prototype Count" value={String(registry.prototypeCount)} />
        <Metric label="Idea Velocity" value={String(registry.ideaVelocity)} />
        <Metric label="Innovation Score" value={String(registry.innovationScore)} />
        <Metric label="Commercial Potential" value={String(registry.commercialPotential)} />
      </section>
      ...
      <section style={gridStyle}>
        {registry.missions.map((mission) => (
          <article key={mission.mission} style={cardStyle}>
            ...
            <p style={mutedStyle}>Score: {mission.innovationScore}</p>
            <Link href={`/innovation/${mission.mission}`} style={linkStyle}>View innovations</Link>
          </article>
        ))}
      </section>
    </main>
