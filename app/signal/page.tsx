...
import { getSignalRegistry } from "../../lib/gamma/signal-reader"
  ...
  return (
    <main style={pageStyle}>
      <section style={cardStyle}>
        ...
        <h1 style={titleStyle}>Signal Detection</h1>
        <p style={mutedStyle}>Trend analysis and anomaly detection.</p>
      </section>

      <section style={metricsStyle}>
        <Metric label="Signal Count" value={String(registry.signalCount)} />
        <Metric label="Trend Score" value={String(registry.trendScore)} />
        <Metric label="Anomaly Count" value={String(registry.anomalyCount)} />
        <Metric label="Priority Signals" value={String(registry.prioritySignals)} />
      ...
      <section style={gridStyle}>
        {registry.missions.map((mission) => (
          <article key={mission.mission} style={cardStyle}>
            ...
            <p style={mutedStyle}>Trend: {mission.trendScore}%</p>
            <Link href={`/signal/${mission.mission}`} style={linkStyle}>View signal</Link>
          </article>
        ))}
      </section>
    </main>
