...
import { getGammaNexusRegistry } from "../../lib/gamma/gamma-nexus-reader"

export const dynamic = "force-dynamic"

export default async function GammaNexusPage() {
  const registry = await getGammaNexusRegistry()

  return (
    <main style={pageStyle}>
      <section style={cardStyle}>
        <p style={eyebrowStyle}>Gamma Nexus</p>
        <h1 style={titleStyle}>Autonomous Enterprise Control</h1>
        <p style={mutedStyle}>Unified command center for Gamma systems.</p>
      </section>
      ...
      <section style={metricsStyle}>
        ...
        <Metric label="Monetization Potential" value={String(registry.monetizationPotential)} />
        <Metric label="Ecosystem Strength" value={String(registry.ecosystemStrength)} />
      </section>

      <section style={metricsStyle}>
        <Metric label="Forecast Confidence" value={String(registry.forecastConfidence)} />
      ...
      <section style={gridStyle}>
        {registry.missions.map((mission) => (
          <article key={mission.mission} style={cardStyle}>
            ...
            <p style={mutedStyle}>Enterprise: {mission.enterpriseScore}</p>
            <Link href={`/gamma-nexus/${mission.mission}`} style={linkStyle}>View nexus</Link>
          </article>
        ))}
      </section>
    </main>
