...
import { getMissionBootstrapRegistry } from "../../lib/gamma/mission-bootstrap-reader"

export const dynamic = "force-dynamic"

export default async function MissionBootstrapPage() {
  const registry = await getMissionBootstrapRegistry()

  return (
    <main style={pageStyle}>
      <section style={cardStyle}>
        <p style={eyebrowStyle}>Mission Duplication Engine</p>
        <h1 style={titleStyle}>Mission Bootstrap</h1>
        <p style={mutedStyle}>Deterministic mission template duplication and workspace bootstrap intelligence.</p>
      </section>

      <section style={metricsStyle}>
        <Metric label="Template Count" value={String(registry.templateCount)} />
        <Metric label="Workspace Count" value={String(registry.workspaceCount)} />
        <Metric label="Generated Assets" value={String(registry.generatedAssets)} />
        <Metric label="Reuse Score" value={String(registry.reuseScore)} />
        <Metric label="Bootstrap Coverage" value={String(registry.bootstrapCoverage)} />
      ...
      <section style={gridStyle}>
        {registry.missions.map((mission) => (
          <article key={mission.mission} style={cardStyle}>
            ...
            <p style={mutedStyle}>Reuse Score: {mission.reuseScore}</p>
            <p style={mutedStyle}>Bootstrap Coverage: {mission.bootstrapCoverage}</p>
            <Link href={`/mission-bootstrap/${mission.mission}`} style={linkStyle}>Open mission bootstrap</Link>
          </article>
        ))}
      </section>
    </main>
