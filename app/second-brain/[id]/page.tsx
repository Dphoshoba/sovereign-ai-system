...
export default async function SecondBrainMissionPage({
  params,
}: {
  params: Promise<{ id: string }>
}) {
  const { id } = await params
  const workspace = await getSecondBrainWorkspace(id)
    ...
    return (
      <main style={pageStyle}>
        <section style={cardStyle}>
          ...
          <p style={mutedStyle}>The requested second brain mission "{id}" does not exist.</p>
        </section>
      </main>
    )
  }
  ...
  return (
    <main style={pageStyle}>
      <section style={headerStyle}>
        <div>
          <p style={eyebrowStyle}>Second Brain Hub</p>
          <h1 style={titleStyle}>{workspace.currentMission}</h1>
          <p style={subtitleStyle}>Unified mission intelligence with priorities, timeline, knowledge growth and reusable assets.</p>
        </div>
      ...
      <section style={grid5Style}>
        <MetricCard label="Missions" value={String(workspace.missionCount)} detail="Mission registry" />
        <MetricCard label="Workspaces" value={String(workspace.workspaceCount)} detail="Connected layers" />
        <MetricCard label="Discoveries" value={String(workspace.discoveryCount)} detail="Aggregated discoveries" />
        <MetricCard label="Assets" value={String(workspace.assetCount)} detail="Reusable assets" />
        <MetricCard label="Priorities" value={String(workspace.priorityCount)} detail="Priority queue" />
      </section>

      <section style={grid2Style}>
        <article style={cardStyle}>
          <h2 style={cardTitleStyle}>Suggested Actions</h2>
        ...
        <article style={cardStyle}>
          <h2 style={cardTitleStyle}>Workspace Status and Progress Indicators</h2>
      ...
      <section style={grid2Style}>
        ...
        <article style={cardStyle}>
          <h2 style={cardTitleStyle}>Knowledge Growth</h2>
          <ul style={listStyle}>
          ...
          <h3 style={subTitleStyle}>Cross Workspace Summary</h3>
          <List items={workspace.crossWorkspaceSummary} />
        </article>
      </section>

      <section style={footerStyle}>
        <Link href="/second-brain" style={linkStyle}>
          Back to Second Brain Hub
        </Link>
      </section>
    </main>
  )
