import Link from "next/link"
import { getMissionReview } from "../../../lib/gamma/review-reader"

export const dynamic = "force-dynamic"

export default async function MissionReviewPage({
  params,
}: {
  params: Promise<{ id: string }>
}) {
  const { id } = await params
  const workspace = await getMissionReview(id)

  if (!workspace) {
    return (
      <main style={pageStyle}>
        <article style={cardStyle}>
          <h1 style={titleStyle}>Review Not Found</h1>
          <p style={mutedStyle}>No review workspace exists for mission {id}.</p>
          <Link href="/review" style={linkStyle}>
            Back to Review
          </Link>
        </article>
      </main>
    )
  }

  return (
    <main style={pageStyle}>
      <section style={cardStyle}>
        <p style={eyebrowStyle}>Weekly Review</p>
        <h1 style={titleStyle}>Research Mission 001</h1>
        <p style={mutedStyle}>{workspace.missionTitle}</p>
        <p style={mutedStyle}>Completed, unfinished, and next-cycle mission outlook.</p>
      </section>

      <section style={metricsGridStyle}>
        <Metric label="Completed" value={String(workspace.completedCount)} />
        <Metric label="Pending" value={String(workspace.pendingCount)} />
        <Metric label="Overdue" value={String(workspace.overdueCount)} />
        <Metric label="Improvement" value={String(workspace.improvementScore)} />
        <Metric label="Regression" value={String(workspace.regressionScore)} />
        <Metric label="Review Score" value={String(workspace.reviewScore)} />
        <Metric label="Momentum" value={String(workspace.momentumScore)} />
        <Metric label="Health" value={String(workspace.healthScore)} />
        <Metric label="Knowledge Debt" value={String(workspace.knowledgeDebt)} />
        <Metric label="Attention" value={String(workspace.attentionScore)} />
        <Metric label="Next Week Actions" value={String(workspace.nextWeekActions.length)} />
        <Metric label="Recommendations" value={String(workspace.recommendationCount)} />
      </section>

      <section style={threeColStyle}>
        <article style={cardStyle}>
          <h2 style={sectionTitleStyle}>Completed</h2>
          <TaskList items={workspace.completed} />
        </article>

        <article style={cardStyle}>
          <h2 style={sectionTitleStyle}>Pending</h2>
          <TaskList items={workspace.pending} />
        </article>

        <article style={cardStyle}>
          <h2 style={sectionTitleStyle}>Overdue</h2>
          <TaskList items={workspace.overdue} />
        </article>
      </section>

      <section style={twoColStyle}>
        <article style={cardStyle}>
          <h2 style={sectionTitleStyle}>Improvements</h2>
          <ul style={listStyle}>
            {workspace.improvements.map((item) => (
              <li key={item} style={listItemStyle}>{item}</li>
            ))}
          </ul>
        </article>

        <article style={cardStyle}>
          <h2 style={sectionTitleStyle}>Regressions</h2>
          <ul style={listStyle}>
            {workspace.regressions.map((item) => (
              <li key={item} style={listItemStyle}>{item}</li>
            ))}
          </ul>
        </article>
      </section>

      <section style={twoColStyle}>
        <article style={cardStyle}>
          <h2 style={sectionTitleStyle}>Next Week</h2>
          <ul style={listStyle}>
            {workspace.nextWeekActions.map((item) => (
              <li key={item} style={listItemStyle}>{item}</li>
            ))}
          </ul>
        </article>

        <article style={cardStyle}>
          <h2 style={sectionTitleStyle}>Attention Needed</h2>
          <ul style={listStyle}>
            {workspace.attentionNeeded.map((item) => (
              <li key={item} style={listItemStyle}>{item}</li>
            ))}
          </ul>
        </article>
      </section>

      <section style={twoColStyle}>
        <article style={cardStyle}>
          <h2 style={sectionTitleStyle}>Knowledge Debt</h2>
          <p style={mutedStyle}>Current debt score: {workspace.knowledgeDebt}</p>
          <h2 style={sectionTitleStyle}>Recommendations</h2>
          <ul style={listStyle}>
            {workspace.recommendations.map((item) => (
              <li key={item} style={listItemStyle}>{item}</li>
            ))}
          </ul>
        </article>

        <article style={cardStyle}>
          <h2 style={sectionTitleStyle}>Mission Outlook</h2>
          <ul style={listStyle}>
            {workspace.missionOutlook.map((item) => (
              <li key={item} style={listItemStyle}>{item}</li>
            ))}
          </ul>
          <h2 style={sectionTitleStyle}>Roadmap</h2>
          <div style={stackStyle}>
            {workspace.roadmap.map((item) => (
              <div key={`${item.date}-${item.event}`} style={timelineRowStyle}>
                <span style={mutedStyle}>{item.date}</span>
                <span>{item.event}</span>
                <span style={mutedStyle}>{item.status}</span>
              </div>
            ))}
          </div>
        </article>
      </section>

      <section style={cardStyle}>
        <Link href="/review" style={linkStyle}>Back to Review</Link>
      </section>
    </main>
  )
}

function Metric({ label, value }: { label: string; value: string }) {
  return (
    <article style={metricCardStyle}>
      <p style={metricLabelStyle}>{label}</p>
      <p style={metricValueStyle}>{value}</p>
    </article>
  )
}

function TaskList({ items }: { items: Array<{ id: string; title: string; workspace: string; priority: string; rationale: string }> }) {
  return (
    <div style={stackStyle}>
      {items.map((item) => (
        <div key={item.id} style={rowStyle}>
          <strong>{item.title}</strong>
          <span style={mutedStyle}>{item.workspace}</span>
          <span style={mutedStyle}>Priority: {item.priority}</span>
          <p style={mutedStyle}>{item.rationale}</p>
        </div>
      ))}
    </div>
  )
}

const pageStyle: React.CSSProperties = { maxWidth: 1450, margin: "0 auto", padding: 32, color: "var(--foreground)" }
const cardStyle: React.CSSProperties = { border: "1px solid var(--border)", borderRadius: 10, padding: 16, background: "var(--card-background)", marginBottom: 14 }
const titleStyle: React.CSSProperties = { margin: "6px 0", fontSize: 32 }
const sectionTitleStyle: React.CSSProperties = { margin: 0, fontSize: 18 }
const eyebrowStyle: React.CSSProperties = { margin: 0, fontSize: 12, color: "var(--muted)", textTransform: "uppercase", letterSpacing: 0.8 }
const mutedStyle: React.CSSProperties = { margin: "5px 0", color: "var(--muted)" }
const metricsGridStyle: React.CSSProperties = { display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(170px, 1fr))", gap: 10, marginBottom: 14 }
const metricCardStyle: React.CSSProperties = { border: "1px solid var(--border)", borderRadius: 8, padding: 12, background: "var(--background)" }
const metricLabelStyle: React.CSSProperties = { margin: 0, fontSize: 12, color: "var(--muted)" }
const metricValueStyle: React.CSSProperties = { margin: "6px 0 0", fontWeight: 700, fontSize: 20 }
const threeColStyle: React.CSSProperties = { display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 12, marginBottom: 14 }
const twoColStyle: React.CSSProperties = { display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(360px, 1fr))", gap: 12, marginBottom: 14 }
const stackStyle: React.CSSProperties = { display: "flex", flexDirection: "column", gap: 8, marginTop: 8 }
const rowStyle: React.CSSProperties = { borderTop: "1px solid var(--border)", paddingTop: 8, display: "flex", flexDirection: "column", gap: 4 }
const timelineRowStyle: React.CSSProperties = { borderTop: "1px solid var(--border)", paddingTop: 8, display: "grid", gridTemplateColumns: "110px 1fr auto", gap: 10 }
const listStyle: React.CSSProperties = { margin: "8px 0 0", paddingLeft: 18 }
const listItemStyle: React.CSSProperties = { marginBottom: 8 }
const linkStyle: React.CSSProperties = { color: "#0284c7", textDecoration: "none", fontWeight: 600 }
