...
import { getMissionSovereignty } from "../../../lib/gamma/sovereignty-reader"
...
export default async function SovereigntyDetailPage({ params }: { params: { id: string } }) {
  const workspace = await getMissionSovereignty(params.id)

  if (!workspace) {
    return (
      <main style={pageStyle}>
        <section style={cardStyle}>
          <h1>Sovereignty not found</h1>
        </section>
      </main>
    )
  }
    ...
    <main style={pageStyle}>
      ...
      <section style={metricsStyle}>
        <div style={metricBoxStyle}>
          ...
          <div style={{ fontSize: "24px", fontWeight: "bold" }}>{workspace.ownershipScore}</div>
        </div>
        <div style={metricBoxStyle}>
          <div style={{ fontSize: "12px", opacity: 0.6 }}>Control Score</div>
          <div style={{ fontSize: "24px", fontWeight: "bold" }}>{workspace.controlScore}</div>
        </div>
        <div style={metricBoxStyle}>
          <div style={{ fontSize: "12px", opacity: 0.6 }}>Dependency Score</div>
          <div style={{ fontSize: "24px", fontWeight: "bold" }}>{workspace.dependencyScore}</div>
        </div>
        <div style={metricBoxStyle}>
        ...
        <div style={metricBoxStyle}>
          ...
          <div style={{ fontSize: "24px", fontWeight: "bold" }}>{workspace.sovereigntyScore}</div>
        </div>
        <div style={metricBoxStyle}>
