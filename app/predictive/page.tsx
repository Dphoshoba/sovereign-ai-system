...
import { getPredictiveRegistry } from "../../lib/gamma/predictive-reader"

export const dynamic = "force-dynamic"

export default async function PredictivePage() {
  const registry = await getPredictiveRegistry()

  return (
    <main style={pageStyle}>
      <section style={cardStyle}>
        <p style={eyebrowStyle}>Predictive Engine</p>
        <h1 style={titleStyle}>Trend Intelligence</h1>
        <p style={mutedStyle}>Forecasts and trend analysis across missions.</p>
      </section>
      ...
      <section style={metricsStyle}>
        ...
        <Metric label="Opportunity Forecast" value={String(registry.opportunityForecast)} />
        <Metric label="Risk Forecast" value={String(registry.riskForecast)} />
      ...
      <section style={gridStyle}>
        {registry.missions.map((mission) => (
          <article key={mission.mission} style={cardStyle}>
            ...
            <Link href={`/predictive/${mission.mission}`} style={linkStyle}>View forecasts</Link>
          </article>
        ))}
      </section>
    </main>
