...
import { getMissionFounderIntelligence } from "../../../lib/gamma/founder-intelligence-reader"

export const dynamic = "force-dynamic"

export default async function FounderIntelligenceMissionPage({
  params,
}: {
  params: Promise<{ id: string }>
}) {
  const { id } = await params
  const workspace = await getMissionFounderIntelligence(id)

  if (!workspace) {
    return (
      <main style={pageStyle}>
        <article style={cardStyle}>
          <h1 style={titleStyle}>Founder Intel Not Found</h1>
          <p style={mutedStyle}>No founder intelligence workspace exists for mission {id}.</p>
          <Link href="/founder-intelligence" style={linkStyle}>Back to Founder Intelligence</Link>
        </article>
      </main>
    )
  }
  ...
  return (
    <main style={pageStyle}>
      ...
      <section style={metricsStyle}>
        ...
        <Metric label="Clarity Score" value={String(workspace.clarityScore)} />
      ...
      <section style={cardStyle}>
        ...
        <ul style={listStyle}>
          {workspace.focuses.map((f) => (
            <li key={f.area} style={listItemStyle}>
              {f.area}: {f.focusLevel}% focus ({f.progress.toFixed(0)}% progress)
            </li>
          ))}
        </ul>
      </section>
