...
import { getOptimizationRegistry } from "../../lib/gamma/optimization-reader"

export const dynamic = "force-dynamic"

export default async function OptimizationPage() {
  const registry = await getOptimizationRegistry()

  return (
    <main style={pageStyle}>
      <section style={cardStyle}>
        <p style={eyebrowStyle}>Optimization Engine</p>
        <h1 style={titleStyle}>System Optimization</h1>
        <p style={mutedStyle}>Efficiency and cost optimization.</p>
      </section>

      <section style={metricsStyle}>
        <Metric label="Optimization Count" value={String(registry.optimizationCount)} />
        <Metric label="Efficiency Score" value={String(registry.efficiencyScore)} />
        <Metric label="Cost Reduction" value={String(registry.costReduction)} />
        <Metric label="Reuse Potential" value={String(registry.reusePotential)} />
        <Metric label="Improvement Velocity" value={String(registry.improvementVelocity)} />
      ...
      <section style={gridStyle}>
        {registry.missions.map((mission) => (
          <article key={mission.mission} style={cardStyle}>
            ...
            <p style={mutedStyle}>Efficiency: {mission.efficiencyScore}%</p>
            <Link href={`/optimization/${mission.mission}`} style={linkStyle}>View optimization</Link>
          </article>
        ))}
      </section>
    </main>
