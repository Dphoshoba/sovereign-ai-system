...
import { getMissionIngestionWorkspace } from "../../../lib/gamma/ingestion-reader"

export const dynamic = "force-dynamic"

export default async function IngestionMissionPage({
  params,
}: {
  params: Promise<{ id: string }>
}) {
  const { id } = await params
  const workspace = await getMissionIngestionWorkspace(id)

  if (!workspace) {
    return (
      <main style={pageStyle}>
        <article style={cardStyle}>
          <h1 style={titleStyle}>Ingestion Workspace Not Found</h1>
          <p style={mutedStyle}>No ingestion workspace exists for mission {id}.</p>
          <Link href="/ingestion" style={linkStyle}>Back to Ingestion</Link>
        </article>
      </main>
    )
  }
  ...
  return (
    <main style={pageStyle}>
      ...
      <section style={metricsStyle}>
        <Metric label="Document Count" value={String(workspace.documentCount)} />
        <Metric label="Mission Count" value={String(workspace.missionCount)} />
        <Metric label="Registry Count" value={String(workspace.registryCount)} />
        <Metric label="Relationship Count" value={String(workspace.relationshipCount)} />
        <Metric label="Graph Node Count" value={String(workspace.graphNodeCount)} />
        <Metric label="Graph Edge Count" value={String(workspace.graphEdgeCount)} />
        <Metric label="Pipeline Coverage" value={String(workspace.pipelineCoverage)} />
        <Metric label="Knowledge Flow Score" value={String(workspace.knowledgeFlowScore)} />
        ...
        <article style={cardStyle}>
          ...
          <ul style={listStyle}>
            {workspace.stages.map((item) => (
              <li key={item.stage} style={listItemStyle}>{item.stage}: {item.source} -&gt; {item.output} ({item.status})</li>
            ))}
          </ul>
        </article>
