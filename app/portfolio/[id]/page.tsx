...
import { getMissionPortfolio } from "../../../lib/gamma/portfolio-reader"

export const dynamic = "force-dynamic"

export default async function PortfolioMissionPage({
  params,
}: {
  params: Promise<{ id: string }>
}) {
  const { id } = await params
  const workspace = await getMissionPortfolio(id)

  if (!workspace) {
    return (
      <main style={pageStyle}>
        <article style={cardStyle}>
          <h1 style={titleStyle}>Portfolio Not Found</h1>
          <p style={mutedStyle}>No portfolio workspace exists for mission {id}.</p>
          <Link href="/portfolio" style={linkStyle}>Back to Portfolio</Link>
        </article>
      </main>
    )
  }
  ...
  return (
    <main style={pageStyle}>
      ...
      <section style={metricsStyle}>
        ...
        <Metric label="Asset Count" value={String(workspace.assetCount)} />
        <Metric label="Workspace Coverage" value={String(workspace.workspaceCoverage)} />
        <Metric label="Portfolio Health" value={String(workspace.portfolioHealth)} />
        ...
        <article style={cardStyle}>
          ...
          <ul style={listStyle}>
            {workspace.coverage.map((item) => (
              <li key={item.workspace} style={listItemStyle}>{item.workspace}: {item.score} ({item.status})</li>
            ))}
          </ul>
        </article>
