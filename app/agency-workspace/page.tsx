...
export default async function AgencyWorkspacePage() {
  const workspace = await getAgencyWorkspace("womanhood")
    ...
    return (
      <main style={pageStyle}>
        <section style={cardStyle}>
          ...
          <p style={mutedStyle}>The requested agency workspace does not exist.</p>
        </section>
      </main>
    )
  }
    ...
    <main style={pageStyle}>
      <section style={headerStyle}>
        <div>
          ...
          <h1 style={titleStyle}>Agency Workspace</h1>
          <p style={subtitleStyle}>Transform research and creator outputs into client-facing deliverables.</p>
        </div>
        <div style={pillColumnStyle}>
          <Pill label="Status" value={workspace.projectStatus} />
          <Pill label="Pipeline" value={workspace.pipelineStage} />
          <Pill label="Confidence" value={workspace.confidence} />
        </div>
      </section>

      <section style={grid4Style}>
        <MetricCard label="Progress" value={`${workspace.progress}%`} detail="Delivery readiness" />
      ...
      <section style={grid4Style}>
        ...
        <MetricCard label="Timeline Entries" value={String(workspace.timelineCount)} detail="Agency timeline" />
        <MetricCard label="Revenue Potential" value={workspace.revenuePotential} detail="Preview estimate" />
        <MetricCard label="Next Suggested Service" value={workspace.nextSuggestedService} detail="Recommended next move" />
      </section>

      <section style={sectionStyle}>
        <h2 style={sectionTitleStyle}>Client Deliverables</h2>
        <div style={grid3Style}>
          <ResourceCard title="Client Proposals" items={workspace.clientProposals} />
          <ResourceCard title="Service Packages" items={workspace.servicePackages} />
          <ResourceCard title="Discovery Documents" items={workspace.discoveryDocuments} />
          <ResourceCard title="Workshop Plans" items={workspace.workshopPlans} />
          <ResourceCard title="Consulting Deliverables" items={workspace.consultingDeliverables} />
          <ResourceCard title="Presentation Outlines" items={workspace.presentationOutlines} />
          <ResourceCard title="Case Studies" items={workspace.caseStudies} />
          <ResourceCard title="Offer Templates" items={workspace.offerTemplates} />
          <ResourceCard title="Roadmaps" items={workspace.roadmaps} />
        </div>
      </section>

      <section style={grid2Style}>
        <article style={cardStyle}>
          <h2 style={cardTitleStyle}>Value Proposition</h2>
          <p style={mutedStyle}>{workspace.valueProposition}</p>
        </article>
        <article style={cardStyle}>
      ...
      <section style={footerStyle}>
        <Link href={`/agency-workspace/${workspace.id}`} style={linkStyle}>
          Open mission agency view
        </Link>
      </section>
    </main>
  )
