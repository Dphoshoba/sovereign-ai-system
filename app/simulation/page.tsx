...
import { getSimulationRegistry } from "../../lib/gamma/simulation-reader"

export const dynamic = "force-dynamic"

export default async function SimulationPage() {
  const registry = await getSimulationRegistry()

  return (
    <main style={pageStyle}>
      <section style={cardStyle}>
        <p style={eyebrowStyle}>Simulation Engine</p>
        <h1 style={titleStyle}>Decision Simulation</h1>
        <p style={mutedStyle}>Monte Carlo simulations and decision modeling.</p>
      </section>

      <section style={metricsStyle}>
        <Metric label="Simulation Count" value={String(registry.simulationCount)} />
        <Metric label="Decision Models" value={String(registry.decisionModels)} />
        <Metric label="Test Coverage" value={String(registry.testCoverage)} />
        <Metric label="Execution Probability" value={String(registry.executionProbability)} />
      ...
      <section style={gridStyle}>
        {registry.missions.map((mission) => (
          <article key={mission.mission} style={cardStyle}>
            ...
            <p style={mutedStyle}>Coverage: {mission.testCoverage}%</p>
            <Link href={`/simulation/${mission.mission}`} style={linkStyle}>View simulation</Link>
          </article>
        ))}
      </section>
    </main>
