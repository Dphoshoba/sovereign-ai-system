...
import { getGrowthRegistry } from "../../lib/gamma/growth-reader"

export const dynamic = "force-dynamic"

export default async function GrowthPage() {
  const registry = await getGrowthRegistry()

  return (
    <main style={pageStyle}>
      <section style={cardStyle}>
        <p style={eyebrowStyle}>Growth Engine</p>
        <h1 style={titleStyle}>Growth Vectors</h1>
        <p style={mutedStyle}>Mission growth potential and expansion strategies.</p>
      </section>
      ...
      <section style={metricsStyle}>
        ...
        <Metric label="Expansion Score" value={String(registry.expansionScore)} />
        <Metric label="Adoption Score" value={String(registry.adoptionScore)} />
        <Metric label="Community Score" value={String(registry.communityScore)} />
      ...
      <section style={gridStyle}>
        {registry.missions.map((mission) => (
          <article key={mission.mission} style={cardStyle}>
            ...
            <p style={mutedStyle}>Potential: {mission.growthPotential}</p>
            <Link href={`/growth/${mission.mission}`} style={linkStyle}>View growth</Link>
          </article>
        ))}
      </section>
    </main>
