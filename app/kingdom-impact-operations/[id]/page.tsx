...
import { getMissionKingdomImpactOperations } from "../../../lib/gamma/kingdom-impact-operations-reader"
...
export default async function KingdomImpactOperationsDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params
  const workspace = await getMissionKingdomImpactOperations(id)
    ...
    <main style={pageStyle}>
      ...
      <section style={metricsStyle}>
        ...
        <div style={metricBoxStyle}>
          ...
          <div style={{ fontSize: "24px", fontWeight: "bold" }}>{workspace.influenceReach}</div>
        </div>
        <div style={metricBoxStyle}>
          <div style={{ fontSize: "12px", opacity: 0.6 }}>Stakeholder Engagement</div>
          <div style={{ fontSize: "24px", fontWeight: "bold" }}>{workspace.stakeholderEngagement}</div>
        </div>
        <div style={metricBoxStyle}>
          <div style={{ fontSize: "12px", opacity: 0.6 }}>Value Creation</div>
          <div style={{ fontSize: "24px", fontWeight: "bold" }}>{workspace.valueCreation}</div>
        </div>
        <div style={metricBoxStyle}>
        ...
        <div style={metricBoxStyle}>
          ...
          <div style={{ fontSize: "24px", fontWeight: "bold" }}>{workspace.legacy}</div>
        </div>
        <div style={metricBoxStyle}>
