...
export async function POST(req: NextRequest) {
  try {
    ...
    const result = recursiveCivilizationInfluenceAgent({
    ...
    console.error("Recursive civilization influence failed:", error)

    return NextResponse.json(
      {
        ok: false,
        error: "Recursive civilization influence failed",
      },
      {
        status: 500,
      }
