...
import { recursivePlanetaryMediaAgent } from "../../../../lib/agents/recursive-planetary-media-agent"
import { recursivePlanetaryCommunityResilienceAgent } from "../../../../lib/agents/recursive-planetary-community-resilience-agent"
import { recursivePlanetaryEthicalAIAgent } from "../../../../lib/agents/recursive-planetary-ethical-ai-agent"
...
export async function POST(req: NextRequest) {
  try {
    ...
    const planetaryEducation = recursivePlanetaryEducationAgent({
    ...
    const planetaryMedia = recursivePlanetaryMediaAgent({
      recursivePlanetaryEducationScore:
        planetaryEducation.recursivePlanetaryEducationScore,
      dominantEducationNetwork:
        planetaryEducation.dominantEducationNetwork.network,
    })

    const communityResilience = recursivePlanetaryCommunityResilienceAgent({
      recursivePlanetaryMediaScore:
        planetaryMedia.recursivePlanetaryMediaScore,
      dominantMediaNetwork:
        planetaryMedia.dominantMediaNetwork.network,
    })

    const ethicalAI = recursivePlanetaryEthicalAIAgent({
      recursivePlanetaryCommunityResilienceScore:
        communityResilience.recursivePlanetaryCommunityResilienceScore,
      dominantResilienceNetwork:
        communityResilience.dominantResilienceNetwork.network,
    })

    return NextResponse.json({
      ok: true,
      phase: "Phase 7",
      phaseSevenComplete: true,
      systemName:
        "Planetary Civilization Infrastructure",
      ...
      phaseSevenSummary: {
        ...
        recursivePlanetaryEthicalAIScore:
          ethicalAI.recursivePlanetaryEthicalAIScore,

        dominantPlanetaryDomain:
          planetaryInfrastructureGenesis
            .dominantPlanetaryDomain.domain,

        dominantEducationNetwork:
          planetaryEducation
            .dominantEducationNetwork.network,

        dominantMediaNetwork:
          planetaryMedia
            .dominantMediaNetwork.network,

        dominantResilienceNetwork:
          communityResilience
            .dominantResilienceNetwork.network,

        dominantEthicalAINetwork:
          ethicalAI
            .dominantEthicalAINetwork.network,

        finalDirective:
          "Scale planetary education, media, resilience and ethical AI infrastructure while preserving human dignity, wisdom, trust and flourishing.",
      },
    ...
    console.error("Phase seven planetary synthesis failed:", error)

    return NextResponse.json(
      {
        ok: false,
        error: "Phase seven planetary synthesis failed",
      },
      { status: 500 }
    )
  }
