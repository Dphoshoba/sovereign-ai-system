...
export async function POST(req: NextRequest) {
  try {
    ...
    const result = recursiveCivilizationForecastingAgent({
    ...
    console.error("Recursive civilization forecasting failed:", error)

    return NextResponse.json(
      {
        ok: false,
        error: "Recursive civilization forecasting failed",
      },
      { status: 500 }
    )
  }
