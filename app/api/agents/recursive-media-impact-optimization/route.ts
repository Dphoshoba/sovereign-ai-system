...
export async function POST(req: NextRequest) {
  try {
    ...
    const result = recursiveMediaImpactOptimizationAgent({
    ...
    console.error("Recursive media impact optimization failed:", error)

    return NextResponse.json(
      {
        ok: false,
        error: "Recursive media impact optimization failed",
      },
      {
        status: 500,
      }
