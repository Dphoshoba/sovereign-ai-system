...
export async function POST(req: NextRequest) {
  try {
    ...
    const result = recursivePlanetaryInfrastructureGenesisAgent({
    ...
    console.error("Recursive planetary infrastructure genesis failed:", error)

    return NextResponse.json(
      {
        ok: false,
        error: "Recursive planetary infrastructure genesis failed",
      },
      { status: 500 }
    )
  }
