...
import { getMissionEnterpriseContinuityOperations } from "../../../lib/gamma/enterprise-continuity-operations-reader"
...
export default async function EnterpriseContinuityOperationsDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params
  const workspace = await getMissionEnterpriseContinuityOperations(id)
    ...
    <main style={pageStyle}>
      ...
      <section style={metricsStyle}>
        ...
        <div style={metricBoxStyle}>
          ...
          <div style={{ fontSize: "24px", fontWeight: "bold" }}>{workspace.businessContinuity}</div>
        </div>
        <div style={metricBoxStyle}>
          <div style={{ fontSize: "12px", opacity: 0.6 }}>Disaster Recovery</div>
          <div style={{ fontSize: "24px", fontWeight: "bold" }}>{workspace.disasterRecovery}</div>
        </div>
        <div style={metricBoxStyle}>
        ...
        <div style={metricBoxStyle}>
          ...
          <div style={{ fontSize: "24px", fontWeight: "bold" }}>{workspace.systemRedundancy}</div>
        </div>
        <div style={metricBoxStyle}>
          <div style={{ fontSize: "12px", opacity: 0.6 }}>Failover Capacity</div>
          <div style={{ fontSize: "24px", fontWeight: "bold" }}>{workspace.failoverCapacity}</div>
        </div>
        <div style={metricBoxStyle}>
