...
import { getSovereignCoreRegistry } from "../../lib/gamma/sovereign-core-reader"
...
export default async function SovereignCorePage() {
  const registry = await getSovereignCoreRegistry()

  return (
    <main style={pageStyle}>
      <section style={cardStyle}>
        <h1>Gamma Sovereign Core</h1>
        <p>Unified multi-system intelligence aggregation</p>
      </section>

      <section style={metricsStyle}>
        <div style={metricBoxStyle}>
          <div style={{ fontSize: "12px", opacity: 0.6 }}>Sovereign Score</div>
          <div style={{ fontSize: "24px", fontWeight: "bold" }}>{registry.sovereignScore}</div>
        </div>
        <div style={metricBoxStyle}>
          <div style={{ fontSize: "12px", opacity: 0.6 }}>Ecosystem Strength</div>
          <div style={{ fontSize: "24px", fontWeight: "bold" }}>{registry.ecosystemStrength}</div>
        </div>
        <div style={metricBoxStyle}>
          <div style={{ fontSize: "12px", opacity: 0.6 }}>Network Strength</div>
          <div style={{ fontSize: "24px", fontWeight: "bold" }}>{registry.networkStrength}</div>
        </div>
        <div style={metricBoxStyle}>
          <div style={{ fontSize: "12px", opacity: 0.6 }}>Mission Capital</div>
          <div style={{ fontSize: "24px", fontWeight: "bold" }}>{registry.missionCapital}</div>
        </div>
        <div style={metricBoxStyle}>
        ...
        <div style={metricBoxStyle}>
          ...
          <div style={{ fontSize: "24px", fontWeight: "bold" }}>{registry.autonomyScore}</div>
        </div>
        <div style={metricBoxStyle}>
        ...
        {registry.missions.map((mission) => (
          <div key={mission.mission} style={missionCardStyle}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", marginTop: "8px" }}>
              <span>Score: {mission.sovereignScore}</span>
