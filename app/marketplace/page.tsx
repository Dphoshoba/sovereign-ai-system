...
import { getMarketplaceRegistry } from "../../lib/gamma/marketplace-reader"
  ...
  return (
    <main style={pageStyle}>
      <section style={cardStyle}>
        ...
        <p>Centralized asset trading and discovery</p>
      </section>

      <section style={metricsStyle}>
        <div style={metricBoxStyle}>
          <div style={{ fontSize: "12px", opacity: 0.6 }}>Asset Count</div>
          <div style={{ fontSize: "24px", fontWeight: "bold" }}>{registry.assetCount}</div>
        </div>
        <div style={metricBoxStyle}>
          <div style={{ fontSize: "12px", opacity: 0.6 }}>Course Count</div>
          <div style={{ fontSize: "24px", fontWeight: "bold" }}>{registry.courseCount}</div>
        </div>
        <div style={metricBoxStyle}>
          <div style={{ fontSize: "12px", opacity: 0.6 }}>Framework Count</div>
          <div style={{ fontSize: "24px", fontWeight: "bold" }}>{registry.frameworkCount}</div>
        </div>
        <div style={metricBoxStyle}>
          <div style={{ fontSize: "12px", opacity: 0.6 }}>Book Count</div>
          <div style={{ fontSize: "24px", fontWeight: "bold" }}>{registry.bookCount}</div>
        </div>
        <div style={metricBoxStyle}>
          <div style={{ fontSize: "12px", opacity: 0.6 }}>Series Count</div>
          <div style={{ fontSize: "24px", fontWeight: "bold" }}>{registry.seriesCount}</div>
        </div>
        <div style={metricBoxStyle}>
          <div style={{ fontSize: "12px", opacity: 0.6 }}>Market Value</div>
          <div style={{ fontSize: "24px", fontWeight: "bold" }}>{registry.marketValue}</div>
        </div>
        <div style={metricBoxStyle}>
        ...
        {registry.missions.map((mission) => (
          <div key={mission.mission} style={missionCardStyle}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", marginTop: "8px" }}>
              <span>Assets: {mission.assetCount}</span>
              <span>Value: {mission.marketValue}</span>
            </div>
          </div>
        ))}
      </section>
